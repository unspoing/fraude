...
class Fraude:
    ...
    def _start(self):
        # fraude label
        bar = tk.Frame(self.root, height=44)
        bar.pack(side = "top", fill="x")
        tk.Label(bar, text="fraude", font=("TkDefaultFont", 13, "bold")).pack(side="left", padx=16, pady=10)
        ...
        # left side
        left = tk.Frame(pane, width=300, relief="flat", bd=1)
        ...
        self.textbox.pack(
            anchor="w", fill="both", expand=True, padx=18, pady=(4, 14)
        )

        # button
        tk.Button(left, text="ponder", command=self._ask).pack(
            fill="both", padx=18, ipady=6)
